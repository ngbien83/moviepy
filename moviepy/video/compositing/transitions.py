...
def slide_out(clip, duration, side):
    ...
    pos_dict = {'left' : lambda t: (min(0,w*(1-(t-t_s)/duration)),'center'),
                'right' : lambda t: (max(0,w*((t-t_s)/duration-1)),'center'),
                'top' : lambda t: ('center',min(0,h*(1-(t-t_s)/duration))),
                'bottom': lambda t: ('center',max(0,h*((t-t_s)/duration-1))) }
